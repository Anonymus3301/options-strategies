// Call Calendar Spread strategy page — standalone, REST-only.
// Sell 1 front-month ATM call, buy 1 back-month call at the same strike — a single-leg,
// cheaper cousin of the straddle Calendar Spread with a mild bullish lean.
...
function computeCalendar() {
  ...
  const strikes = [...frontBucket.calls.keys()];
  const strike = qClosestStrike(strikes, spot);
  if (strike == null) return { spot };
  if (!backBucket.calls.has(strike)) return { spot, strike, noBackMatch: true };
  ...
  if (!frontCall || !backCall || frontCall.mark_price == null || backCall.mark_price == null) {
    return { spot, strike };
  }
...
function makePnlAt(strike, remainingT, sigmaBack, frontUsd, backUsd) {
  return (S) => frontUsd - Math.max(S - strike, 0) - backUsd + qBsPrice("call", S, strike, remainingT, sigmaBack);
}
