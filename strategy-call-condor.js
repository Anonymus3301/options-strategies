// Call Condor Spread strategy page — standalone, REST-only.
// Buy 1 call at K1, sell 1 call at K2, sell 1 call at K3, buy 1 call at K4 — a wider,
// flatter-topped cousin of the Butterfly Spread. With equal wing widths both tails and
// the whole flat top are exact constants (verified numerically before shipping).
...
function computeCondor(bodyPct, wingPct) {
  ...
  const call4 = state.summaries.get(bucket.calls.get(k4));
  if (
    !call1 || !call2 || !call3 || !call4 ||
    call1.mark_price == null || call2.mark_price == null || call3.mark_price == null || call4.mark_price == null
  ) {
    return { spot, k1, k2, k3, k4 };
  }

  const usd1 = call1.mark_price * spot, usd2 = call2.mark_price * spot;
  const usd3 = call3.mark_price * spot, usd4 = call4.mark_price * spot;
  const legs = [
    { type: "call", side: "long", strike: k1, premiumUsd: usd1 },
    { type: "call", side: "short", strike: k2, premiumUsd: usd2 },
    { type: "call", side: "short", strike: k3, premiumUsd: usd3 },
    { type: "call", side: "long", strike: k4, premiumUsd: usd4 },
  ];
  const netDebit = usd1 + usd4 - usd2 - usd3;
  const maxProfit = qLegsPnlAt(legs, (k2 + k3) / 2);
  const ivs = [call2.mark_iv, call3.mark_iv].filter((v) => v != null);
