// Covered Put Overwrite (2:1 Ratio Write) strategy page — standalone, REST-only.
// For an assumed short BTC position: sell 2 OTM puts per 1 BTC short instead of 1 (a plain
// covered put). Above the strike this is unchanged from a plain covered put (unlimited
// loss on the short as price rises — nothing new). Below the strike, the extra naked put
// flips what would otherwise be a flat, closed-out position into net LONG one put, with
// loss that grows the further price falls. Verified numerically (scratchpad): payoff slope
// below the strike is exactly +1 for the 2:1 write vs. exactly 0 for a plain covered put.
...
function computeRatioWrite(widthPct) {
  ...
  const maxProfit = spot - putStrike + 2 * premiumUsd;
  const upsideBreakeven = spot + 2 * premiumUsd;
  const amplifiedRiskBreakeven = 2 * putStrike - spot - 2 * premiumUsd;

  return { spot, putStrike, premiumUsd, maxProfit, upsideBreakeven, amplifiedRiskBreakeven, iv: put.mark_iv };
}
...
function renderPayoff(rw) {
  ...
  const popEl = $("popStat");
  if (!rw || rw.spot == null || rw.putStrike == null || rw.premiumUsd == null) {
    el.innerHTML = '<p class="loading">No data</p>';
    $("strikeStat").textContent = "—";
    $("premiumStat").textContent = "—";
    $("maxProfitStat").textContent = "—";
    $("upsideBeStat").textContent = "—";
    $("riskBeStat").textContent = "—";
    if (popEl) popEl.textContent = "—";
    return;
  }

  const { spot, putStrike, premiumUsd, maxProfit, upsideBreakeven, amplifiedRiskBreakeven, iv } = rw;
  ...
  $("upsideBeStat").textContent = qFmt(upsideBreakeven, 0);
  $("riskBeStat").textContent = qFmt(amplifiedRiskBreakeven, 0);

  const unhedgedPnl = (S) => spot - S;
  const coveredPutPnl = (S) => (spot - S) + premiumUsd - Math.max(putStrike - S, 0);
  const ratioWritePnl = (S) => (spot - S) + 2 * premiumUsd - 2 * Math.max(putStrike - S, 0);
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    pts.push({ S, unhedged: unhedgedPnl(S), covered: coveredPutPnl(S), ratio: ratioWritePnl(S) });
  }
