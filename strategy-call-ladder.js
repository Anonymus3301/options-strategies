// Call Ladder (Christmas Tree) strategy page — standalone, REST-only.
// Buy 1 near-ATM call (K1), sell 1 further-OTM call (K2), sell 1 even further-OTM call
// (K3). Selling the second call finances the trade further than a plain bull call spread,
// at the cost of turning into an uncapped short position above K3.
...
function computeLadder(innerPct, outerPct) {
  ...
  const call1 = state.summaries.get(bucket.calls.get(k1));
  const call2 = state.summaries.get(bucket.calls.get(k2));
  const call3 = state.summaries.get(bucket.calls.get(k3));
  if (!call1 || !call2 || !call3 || call1.mark_price == null || call2.mark_price == null || call3.mark_price == null) {
    return { spot, k1, k2, k3 };
  }

  const usd1 = call1.mark_price * spot;
  const usd2 = call2.mark_price * spot;
  const usd3 = call3.mark_price * spot;
  const netCost = usd1 - usd2 - usd3; // positive = net debit, negative = net credit
  const maxProfit = (k2 - k1) - netCost;
  const breakevenLow = netCost > 0 ? k1 + netCost : null;
  const breakevenHigh = k3 + maxProfit;
  const ivs = [call1.mark_iv, call2.mark_iv, call3.mark_iv].filter((v) => v != null);
  const avgIv = ivs.length ? ivs.reduce((a, b) => a + b, 0) / ivs.length : null;

  return { spot, k1, k2, k3, usd1, usd2, usd3, netCost, maxProfit, breakevenLow, breakevenHigh, avgIv };
}
...
function renderLadder(l) {
  ...
  $("breakevenStat").textContent =
    l.breakevenLow != null
      ? `${qFmt(l.breakevenLow, 0)} — ${qFmt(l.breakevenHigh, 0)} (unlimited loss above)`
      : `None below K2 (net credit) — ${qFmt(l.breakevenHigh, 0)} upper (unlimited loss above)`;

  const legs = [
    { type: "call", side: "long", strike: l.k1, premiumUsd: l.usd1 },
    { type: "call", side: "short", strike: l.k2, premiumUsd: l.usd2 },
    { type: "call", side: "short", strike: l.k3, premiumUsd: l.usd3 },
  ];
