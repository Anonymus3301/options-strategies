// Poor Man's Covered Put (PMCP) strategy page — standalone, REST-only.
// Buy a deep ITM put at the furthest listed expiry (a high-delta stand-in for a short BTC
// position), sell a near-dated OTM put against it. The bearish mirror of the PMCC page.
...
function findDeltaStrike(strikes, bucket, spot, T, targetDelta) {
  ...
  for (const strike of strikes) {
    const name = bucket.puts.get(strike);
...
function computePmcp(backDeltaMag, frontDeltaMag) {
  ...
  const longLeg = findDeltaStrike(backStrikes, backBucket, spot, Tback, -backDeltaMag);
  if (!longLeg) return { spot };

  const frontStrikes = [...frontBucket.puts.keys()].sort((a, b) => a - b);
  const shortLeg = findDeltaStrike(frontStrikes, frontBucket, spot, Tfront, -frontDeltaMag);
  ...
  const pnlAt = (S) => shortUsd - Math.max(shortLeg.strike - S, 0) - longUsd + qBsPrice("put", S, longLeg.strike, remainingT, sigmaBack);
...
function buildPmcpPayoffSvg(pnlAt, spot, strikes) {
...
function render(p) {
  ...
  $("strikesStat").textContent = `${qFmt(p.shortLeg.strike, 0)} (Δ${qFmt(p.shortLeg.delta, 2)}) / ${qFmt(p.longLeg.strike, 0)} (Δ${qFmt(p.longLeg.delta, 2)})`;
  ...
  chartEl.innerHTML = buildPmcpPayoffSvg(p.pnlAt, p.spot, [p.longLeg.strike, p.shortLeg.strike]);

  const pop = qComputeProbabilityOfProfitFn(p.pnlAt, p.spot, Math.abs(p.shortLeg.iv) / 100, p.Tfront);
...
async function refresh() {
  try {
    ...
    await loadChain();
    renderFrontSelect();
    const backDeltaMag = Number($("backDeltaSelect").value);
    const frontDeltaMag = Number($("frontDeltaSelect").value);
    const p = computePmcp(backDeltaMag, frontDeltaMag);
    render(p);
